...
| cronExpression | varchar(255) |  | true |  |  | Cron expression driving recurrence; set when kind is 'cron' or 'recurring_cron' (where it is the anchor the recurrence gate thins). |
...
| recurrenceSize | integer |  | true |  |  | The N of the every-Nth-period recurrence gate, at least 2; set only when kind is 'recurring_cron'. |
| recurrenceUnit | varchar(16) |  | true |  |  | Calendar period of the every-Nth-period recurrence gate; set only when kind is 'recurring_cron'. |
